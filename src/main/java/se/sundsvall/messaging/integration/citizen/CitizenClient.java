...
@FeignClient(name = INTEGRATION_NAME, url = "${integration.citizen.base-url}", configuration = CitizenIntegrationConfiguration.class)
interface CitizenClient {

	@GetMapping(path = "/{municipalityId}/{personId}?ShowClassified=false", produces = APPLICATION_JSON_VALUE)
	Optional<CitizenExtended> getCitizen(@PathVariable(name = "municipalityId") String municipalityId,
