...
public class MessageProcessor extends Processor {
    ...
    public void handleIncomingMessageEvent(final IncomingMessageEvent event) {
        ...
        var feedbackSettings = feedbackSettingsIntegration.getSettingsByPartyId(partyId);
        if (feedbackSettings.isEmpty()) {
            ...
            for (var feedbackSetting : feedbackSettings) {
                for (var feedbackChannel : feedbackSetting.getChannels()) {
                    ...
                    switch (actualContactMethod) {
                        case EMAIL -> {
                            log.info("Moving incoming message {} to e-mail", message.getMessageId());
                        ...
                        case SMS -> {
                            log.info("Moving incoming message {} to SMS", message.getMessageId());
