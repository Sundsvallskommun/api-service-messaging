...
class JsonUtilsTest {
	...
	void fromJsonWithInvalidJsonUsingClass() {
		assertThatExceptionOfType(JacksonException.class).isThrownBy(() -> JsonUtils.fromJson("{", Person.class));
	}
	...
	void fromJsonWithInvalidJsonUsingTypeReference() {
		final var typeRef = new TypeReference<Person>() {
		};

		assertThatExceptionOfType(JacksonException.class).isThrownBy(() -> JsonUtils.fromJson("{", typeRef));
	}
