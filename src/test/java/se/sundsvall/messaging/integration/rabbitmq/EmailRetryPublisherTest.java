...
class EmailRetryPublisherTest {
	...
	private static final String RETRY_EXCHANGE = "api-fabriken.messaging.email.retry";
	...
	private EmailRetryPublisher publisher() {
		...
		return new EmailRetryPublisher(mockRabbitTemplate, properties());
	}
	...
	void publishGiveUp_usesTheDeadKey() {
		publisher().publishGiveUp(emailQueueMessage(), "no more tries");

		verify(mockRabbitTemplate).convertAndSend(eq(RETRY_EXCHANGE), eq("email.dead"), any(Object.class),
			any(MessagePostProcessor.class), any(CorrelationData.class));
	}
