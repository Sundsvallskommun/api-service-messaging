...
class MessageResourceSnailMailBatchFailureTest {
	...
	void shouldFailWithInvalidMunicipalityId() {
		final var response = webTestClient.post()
			...
			.contentType(MediaType.APPLICATION_JSON).exchange().expectStatus().isEqualTo(HttpStatus.BAD_REQUEST)
		...
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message).containsExactly(
			tuple("triggerSnailMailBatchProcessing.municipalityId", "not a valid municipality ID"));
	}

	@Test
	void shouldFailWithInvalidBatchId() {
		final var response = webTestClient.post().uri("/2281/snail-mail/batch/invalid-batchId")
			.contentType(MediaType.APPLICATION_JSON).exchange().expectStatus().isEqualTo(HttpStatus.BAD_REQUEST)
			.expectHeader().contentType(APPLICATION_PROBLEM_JSON).expectBody(ConstraintViolationProblem.class)
			.returnResult().getResponseBody();
