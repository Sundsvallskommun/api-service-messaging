...
class DeprecatedMessageResourceDigitalMailTest {
	...
	void sendSynchronous(boolean hasSender, boolean includeOptionalHeaders) {
		...
		verify(mockMessageService).sendDigitalMail(includeOptionalHeaders ? addHeaderValues(decoratedRequest) : decoratedRequest, ORGANIZATION_NUMBER);
	...
	void testOldHeadersShouldBePreserved() {
		...
		verify(mockMessageService).sendDigitalMail(decoratedRequest.withOrigin(X_ORIGIN_HEADER_VALUE).withIssuer(X_ISSUER_HEADER_VALUE), ORGANIZATION_NUMBER);
		verifyNoMoreInteractions(mockEventDispatcher);
		verifyNoInteractions(mockEventDispatcher);
	}
