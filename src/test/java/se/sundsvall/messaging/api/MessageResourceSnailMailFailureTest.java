...
class MessageResourceSnailMailFailureTest {
	...
	void shouldFailWithInvalidMunicipalityId() {
		final var validRequest = createValidSnailMailRequest();
		final var response = webTestClient.post()
			.uri("/invalid-municipalityId/snail-mail?batchId=f427952b-247c-4d3b-b081-675a467b3619")
			.contentType(MediaType.APPLICATION_JSON).bodyValue(validRequest).exchange().expectStatus()
			.isEqualTo(HttpStatus.BAD_REQUEST).expectHeader().contentType(APPLICATION_PROBLEM_JSON)
	...
	void shouldFailWithInvalidBatchId() {
		final var validRequest = createValidSnailMailRequest();
		final var response = webTestClient.post().uri("/2281/snail-mail?batchId=invalid-batchId")
			.contentType(MediaType.APPLICATION_JSON).bodyValue(validRequest).exchange().expectStatus()
			.isEqualTo(HttpStatus.BAD_REQUEST).expectHeader().contentType(APPLICATION_PROBLEM_JSON)
			.expectBody(ConstraintViolationProblem.class).returnResult().getResponseBody();
	...
	void shouldFailWithEmptyRequestBody() {
		final var emptyBody = SnailMailRequest.builder().build();
		final var response = webTestClient.post().uri("/2281/snail-mail?batchId=f427952b-247c-4d3b-b081-675a467b3619")
			.contentType(MediaType.APPLICATION_JSON).bodyValue(emptyBody).exchange().expectStatus()
			.isEqualTo(HttpStatus.BAD_REQUEST).expectHeader().contentType(APPLICATION_PROBLEM_JSON)
			.expectBody(ConstraintViolationProblem.class).returnResult().getResponseBody();
	...
	void shouldFailWithInvalidFolderName() {
		final var invalidRequest = createValidSnailMailRequest().withFolderName("questionMarkIsInvalid_?");

		final var response = webTestClient.post().uri("/2281/snail-mail?batchId=f427952b-247c-4d3b-b081-675a467b3619")
			.contentType(MediaType.APPLICATION_JSON).bodyValue(invalidRequest).exchange().expectStatus()
			.isEqualTo(HttpStatus.BAD_REQUEST).expectHeader().contentType(APPLICATION_PROBLEM_JSON)
			.expectBody(ConstraintViolationProblem.class).returnResult().getResponseBody();
