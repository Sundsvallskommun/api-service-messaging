...
class MessageResourceEmailBatchTest {
	...
	private static final InternalDeliveryResult DELIVERY_RESULT = InternalDeliveryResult.builder()
		.withMessageId("someMessageId").withDeliveryId("someDeliveryId").withMessageType(EMAIL).withStatus(SENT)
		.build();

	private static final InternalDeliveryBatchResult DELIVERY_BATCH_RESULT = InternalDeliveryBatchResult.builder()
		.withBatchId("someBatchId").withDeliveries(List.of(DELIVERY_RESULT)).withMunicipalityId(MUNICIPALITY_ID)
		.build();
	...
	private static EmailBatchRequest addHeaderValues(final EmailBatchRequest request) {
	...
	void sendBatch(final boolean includeOptionalHeaders) {
		// Arrange
		...
		final var response = webTestClient.post().uri(URL).headers(handleHeaders(includeOptionalHeaders))
			...
			.expectHeader().valuesMatch(LOCATION, "^/2281/status/batch/(.*)$").expectStatus().isCreated()
		...
		verify(mockEventDispatcher)
			.handleEmailBatchRequest(includeOptionalHeaders ? addHeaderValues(decoratedRequest) : decoratedRequest);
	...
	void testOldHeadersShouldBePreserved() {
		...
		final var response = webTestClient.post().uri(URL).headers(oldHeaders()).contentType(APPLICATION_JSON)
			...
			.valuesMatch(LOCATION, "^/2281/status/batch/(.*)$").expectStatus().isCreated()
			.expectBody(MessageBatchResult.class).returnResult().getResponseBody();
		...
		verify(mockEventDispatcher).handleEmailBatchRequest(
