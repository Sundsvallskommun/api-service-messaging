...
/**
 * There's a couple of cases to cover for the metadata history, we test them all here. Since it's messy to compare the
 * sent in object {@link History} with the response object {@link HistoryResponse} we use the method toHistoryResponse
 * that doesn't remove the file attachment fields to compare the two objects.
 */
...
class HistoryMetadataTest {
	...
	void testEmailRequest_toMetadataHistoryResponse() {
		...
		assertThat(historyResponseWithoutFileContent).usingRecursiveComparison().ignoringFields("content")
		...
		assertThat(emailRequest.attachments()).extracting(EmailRequest.Attachment::name,
			EmailRequest.Attachment::contentType, EmailRequest.Attachment::content).containsExactlyInAnyOrder(
				tuple("test.txt", "text/plain", null), tuple("test2.txt", "text/plain", null));
	}
	...
	void testSmsRequest_toMetadataHistoryResponse() {
		...
		assertThat(historyWithFileContent).usingRecursiveComparison().isEqualTo(historyWithoutFileContent);
	}
	...
	void testWebMessage_toMetadataHistoryResponse() {
		...
		assertThat(historyWithFileContent).usingRecursiveComparison().ignoringFieldsMatchingRegexes(".*\\.base64Data")// Ignore
																														// all
																														// base64Data
																														// fields
		...
		assertThat(webMessageRequest.attachments())
			.extracting(WebMessageRequest.Attachment::fileName, WebMessageRequest.Attachment::mimeType,
				WebMessageRequest.Attachment::base64Data)
			.containsExactlyInAnyOrder(tuple("string", "string", null), tuple("string2", "string2", null));
	}
	...
	void testDigitalMailRequest_toMetadataHistoryResponse() {
		...
		// Check that the two objects are equal, ignoring the content field
		assertThat(historyWithFileContent).usingRecursiveComparison().ignoringFieldsMatchingRegexes(".*\\.content")// Ignore
																													// all
																													// content
																													// fields
			.isEqualTo(historyWithoutFileContent);
		...
		assertThat(digitalMailRequest.attachments())
			.extracting(DigitalMailRequest.Attachment::filename, DigitalMailRequest.Attachment::contentType,
				DigitalMailRequest.Attachment::content)
			.containsExactlyInAnyOrder(tuple("someFileName.pdf", "application/pdf", null),
	...
	void testDigitalInvoice_toMetadataHistoryResponse() {
		...
		assertThat(digitalInvoiceRequest.files()).extracting(DigitalInvoiceRequest.File::filename,
			DigitalInvoiceRequest.File::contentType, DigitalInvoiceRequest.File::content).containsExactlyInAnyOrder(
				tuple("string", "application/pdf", null), tuple("string2", "application/pdf", null));
	}
	...
	void testSnailMailRequest_toMetadataHistoryResponse() {
		...
		assertThat(snailMailRequest.attachments())
			.extracting(SnailMailRequest.Attachment::filename, SnailMailRequest.Attachment::contentType,
				SnailMailRequest.Attachment::content)
			.containsExactlyInAnyOrder(tuple("test.pdf", "application/pdf", null),
	...
	void testLetterRequest_toMetadataHistoryResponse() {
		...
		assertThat(letterRequest.attachments()).extracting(LetterRequest.Attachment::filename,
			LetterRequest.Attachment::contentType, LetterRequest.Attachment::content).containsExactlyInAnyOrder(
				tuple("string", "application/pdf", null), tuple("string2", "application/pdf", null));
	}
	...
	private static History.HistoryBuilder createHistoryBase(MessageType messageType) {
		return History.builder().withBatchId("batchId").withMessageId("messageId").withDeliveryId("deliveryId")
			.withMessageType(messageType).withOriginalMessageType(messageType).withStatus(MessageStatus.SENT)
			.withOrigin("origin").withIssuer("issuer").withCreatedAt(LocalDateTime.now())
			.withMunicipalityId(MUNICIPALITY_ID);
	}

	private static History createEmailHistory() {
		return createHistoryBase(MessageType.EMAIL).withContent("""
			{
			  "party": {
	...
	public static History createSmsHistory() {
		return createHistoryBase(MessageType.SMS).withContent("""
			{
			  "party": {
	...
	public static History createWebMessageHistory() {
		return createHistoryBase(MessageType.WEB_MESSAGE).withContent("""
			{
			  "party": {
			    "partyId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
			    "externalReferences": [
	...
	public static History createDigitalMailHistory() {
		return createHistoryBase(MessageType.DIGITAL_MAIL).withContent("""
			{
			    "attachments": [
			        {
	...
	private static History createDigitalInvoiceHistory() {
		return createHistoryBase(MessageType.DIGITAL_INVOICE).withContent("""
			{
			  "party": {
			    "partyId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
			    "externalReferences": [
	...
	private static History createMessageHistory() {
		return createHistoryBase(MessageType.MESSAGE).withContent("""
			{
			   "messages": [
			     {
			       "party": {
	...
	private static History createSnailMailHistory() {
		return createHistoryBase(MessageType.SNAIL_MAIL).withContent("""
			{
			  "party": {
			    "partyIds": [
			      "3fa85f64-5717-4562-b3fc-2c963f66afa6"
	...
	private static History createLetterHistory() {
		return createHistoryBase(MessageType.LETTER).withContent("""
			{
			  "party": {
			    "partyIds": [
			      "3fa85f64-5717-4562-b3fc-2c963f66afa6"
	...
	private static History createSlackHistory() {
		return createHistoryBase(MessageType.SLACK).withContent("""
			{
			   "token": "string",
			   "channel": "string",
			   "message": "string"
