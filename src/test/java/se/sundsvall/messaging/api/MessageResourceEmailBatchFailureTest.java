...
class MessageResourceEmailBatchFailureTest {
	...
	private static Stream<Arguments> emailBatchRequestBadRequestProvider() {
		return Stream.of(Arguments.of(REQUEST.withSubject(null), "subject", "must not be blank"),
			...
			Arguments.of(REQUEST.withParties(List.of(PARTY.withEmailAddress(null))), "parties[0].emailAddress",
				"must not be blank"),
			Arguments.of(REQUEST.withParties(List.of(PARTY.withEmailAddress(""))), "parties[0].emailAddress",
				"must not be blank"),
			Arguments.of(REQUEST.withParties(List.of(PARTY.withEmailAddress("null"))), "parties[0].emailAddress",
				"must be a well-formed email address"),
			Arguments.of(REQUEST.withParties(List.of(PARTY.withPartyId("not uuid"))), "parties[0].partyId",
				"not a valid UUID"),
			Arguments.of(REQUEST.withSender(SENDER.withReplyTo("null")), "sender.replyTo",
				"must be a well-formed email address"),
			...
			Arguments.of(REQUEST.withSender(SENDER.withAddress("not an email")), "sender.address",
				"must be a well-formed email address"),
			Arguments.of(REQUEST.withAttachments(List.of(ATTACHMENT.withContent(null))), "attachments[0].content",
				"not a valid BASE64-encoded string"),
			Arguments.of(REQUEST.withAttachments(List.of(ATTACHMENT.withContent("not base64"))),
				"attachments[0].content", "not a valid BASE64-encoded string"),
			Arguments.of(REQUEST.withAttachments(List.of(ATTACHMENT.withName(null))), "attachments[0].name",
				"must not be blank"),
			Arguments.of(REQUEST.withAttachments(List.of(ATTACHMENT.withName(""))), "attachments[0].name",
				"must not be blank"),
			Arguments.of(REQUEST.withHeaders(Map.of(Header.MESSAGE_ID.name(), List.of("not a valid message id"))),
				"headers.MESSAGE_ID", "must start with '<', contain '@' and end with '>'"),
			Arguments.of(REQUEST.withHeaders(Map.of(Header.IN_REPLY_TO.name(), List.of("not a valid in reply to"))),
				"headers.IN_REPLY_TO", "must start with '<', contain '@' and end with '>'"),
			Arguments.of(REQUEST.withHeaders(Map.of(Header.REFERENCES.name(), List.of("not a valid reference"))),
				"headers.REFERENCES", "must start with '<', contain '@' and end with '>'"),
			Arguments.of(
				REQUEST.withHeaders(
					Map.of(Header.AUTO_SUBMITTED.name(), List.of("not a valid auto-submitted value"))),
				"headers.AUTO_SUBMITTED", "must be equal to 'auto-generated'"));
	}
	...
	void sendBatch(final EmailBatchRequest request, final String field, final String message) {
		final var response = webTestClient.post().uri(URL).bodyValue(request).exchange().expectStatus().isBadRequest()
