...
class MessageResourceSnailMailTest {
	...
	void sendSnailMail_shouldReturnCreated() {
		...
		when(mockMessageService.sendSnailMail(any(), any())).thenReturn(new InternalDeliveryResult("messageId",
			"deliveryId", MessageType.SNAIL_MAIL, MessageStatus.SENT, MUNICIPALITY_ID));

		final var response = webTestClient.post().uri("/2281/snail-mail?batchId=f427952b-247c-4d3b-b081-675a467b3619")
			.contentType(MediaType.APPLICATION_JSON).bodyValue(validRequest)
			.header(Identifier.HEADER_NAME, "test01user; type=adAccount").exchange().expectHeader().exists(LOCATION)
	...
	void triggerSnailMailBatch_shouldReturnOK() {
		webTestClient.post().uri("/2281/snail-mail/batch/f427952b-247c-4d3b-b081-675a467b3619")
			.contentType(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk();
