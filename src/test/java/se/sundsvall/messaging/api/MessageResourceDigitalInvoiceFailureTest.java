...
class MessageResourceDigitalInvoiceFailureTest {
	...
	@ValueSource(strings = " ")
	@NullAndEmptySource
	void shouldFailWithDetailsWithInvalidPaymentReference(final String reference) {
		// Arrange
	...
	@ValueSource(strings = " ")
	@NullAndEmptySource
	void shouldFailWithDetailsWithInvalidAccountNumber(final String accountNumber) {
		// Arrange
	...
	void shouldFailWithFileWithInvalidContentType(final String contentType) {
		// Arrange
		final var request = validRequest
			.withFiles(List.of(validRequest.files().getFirst().withContentType(contentType)));
