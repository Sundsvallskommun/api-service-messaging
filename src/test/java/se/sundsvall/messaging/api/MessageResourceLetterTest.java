...
class MessageResourceLetterTest {
	...
	private static final InternalDeliveryResult DELIVERY_RESULT = InternalDeliveryResult.builder()
		.withMessageId("someMessageId").withDeliveryId("someDeliveryId").withMessageType(LETTER)
	...
	private static LetterRequest addHeaderValues(final LetterRequest request) {
	...
	void sendSynchronous(final boolean hasSender, final boolean includeOptionalHeaders) {
		...
		verify(mockMessageService).sendLetter(
	...
	void sendAsynchronous(final boolean hasSender, final boolean includeOptionalHeaders) {
		...
		final var request = hasSender ? createValidLetterRequest() : createValidLetterRequest().withSender(null);
		final var decoratedRequest = request.withMunicipalityId(MUNICIPALITY_ID);
		when(mockEventDispatcher.handleLetterRequest(any(LetterRequest.class), anyString()))
			.thenReturn(DELIVERY_BATCH_RESULT);
		...
		verify(mockEventDispatcher).handleLetterRequest(
			includeOptionalHeaders ? addHeaderValues(decoratedRequest) : decoratedRequest, ORGANIZATION_NUMBER);
		verifyNoMoreInteractions(mockEventDispatcher);
