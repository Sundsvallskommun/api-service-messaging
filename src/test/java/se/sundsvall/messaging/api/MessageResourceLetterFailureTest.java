...
class MessageResourceLetterFailureTest {
	...
	@BeforeEach
	void setupRequest() {
		validRequest = createValidLetterRequest();
	}
	...
	@ValueSource(strings = " ")
	@NullAndEmptySource
	void shouldFailWithMissingDepartment(final String department) {
		// Arrange
		final var request = validRequest.withDepartment(department);
	...
	@Test
	void shouldFailWithEmptyAttachements() {
		// Arrange
		final var request = validRequest.withAttachments(emptyList());
		...
		assertThat(response).isNotNull();
		assertThat(response.getViolations()).extracting(Violation::field, Violation::message)
			.containsExactly(tuple("attachments", "must not be empty"));
